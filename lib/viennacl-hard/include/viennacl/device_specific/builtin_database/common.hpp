...
inline ParamT const & get_parameters(database_type<ParamT> const & database, viennacl::ocl::device const & device)
{
  ...
  if (vendor_it==database.map.d.end()) {
    std::cout << "vendor ID not in database" << std::endl;
    auto res = database.at(ocl::unknown_id, dev_type, ocl::unknown, "", numeric_type);
    //res = viennacl::device_specific::matrix_product_parameters(1, 8, 16, 8, 4, 4, 4, FETCH_FROM_LOCAL, FETCH_FROM_LOCAL, 16, 16);
    res = viennacl::device_specific::matrix_product_parameters(1, 8, 16, 8, 4, 4, 4, FETCH_FROM_GLOBAL_STRIDED, FETCH_FROM_GLOBAL_STRIDED, 0, 0);
    return res;
  }
  ...
  if (device_type_it==vendor_it->second.d.end()) {
    std::cout << "device type not in database" << std::endl;    
  ...
  if (expression_it==device_name_it->second.d.end()) {
    std::cout << "expression name not in database" << std::endl;    
    return database.at(ocl::unknown_id, dev_type, ocl::unknown, "", numeric_type);
  }
